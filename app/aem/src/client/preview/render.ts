...
const createRuntime = (wcmmode: any, content: Object, resourceLoaderPath: string, components: any [], models: Object) => {
  return new Runtime()
    ...
    .withResourceLoader(new ResourceResolver(content || {}, new ComponentLoader(), components).createResourceLoader(resourceLoaderPath || '/'));
};
...
const getDecorationElement = (decorationTag: DecorationTag, element: Element) => {
  ...
  if (decorationTag) {
    ...
    typeof element === TYPE_STRING ? decorationElement.innerHTML = element : decorationElement.appendChild(element);
  }
...
const shouldRemount = (forceRender: boolean, element: Element, decorationElement: Element) => {
  return !(forceRender === true &&
    (
      ...
      ((element && element.outerHTML && ROOT_ELEMENT.firstChild) &&
        (ROOT_ELEMENT.firstChild.outerHTML === element.outerHTML ||
        decorationElement && ROOT_ELEMENT.firstChild.outerHTML === decorationElement.outerHTML)
      )
    )
  )
}
  ...
  if (ROOT_ELEMENT && element instanceof Node !== false || typeof element === TYPE_STRING) {
    ...
    if (shouldRemount(forceRender, element, decorationElement)) {
      ...
      if (decorationTag) {
        ...
        typeof element === TYPE_STRING ? ROOT_ELEMENT.innerHTML = element : ROOT_ELEMENT.appendChild(element);
      }
    }
  } else {
