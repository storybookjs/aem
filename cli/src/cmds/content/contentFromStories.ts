...
export const createContentFromStories = async config => {
  ...
  log(`Checking to see if '${config.component.name}' Component Page exists in AEM...`);
  const pageCheckResponse = await fetchFromAEM({
    url: `${config.aemContentPath}/${config.component.name}.infinity.json`,
    method: 'GET',
  ...
  if (await pageCheckResponse.ok) {
    ...
    log(`Component Page for '${config.component.name}' component exists.`);
    ...
    if (await pageCreation) {
      log(`Component page created for '${config.component.name}' component.`);
      log(`Creating JCR:Content for '${config.component.name}' component. Waiting...`);

      const jcrContentCreation = await createPageJCRContent(config);
      if (await jcrContentCreation) {
        log(`Creating JCR:Content for '${config.component.name}' component succeeded.`);
        log(`Creating content for stories...`);
        const storyCreationStatus = await createStories(config);
        if (await storyCreationStatus) {
          log(`Stories created successfully.`);
          log(
            `Story content -> http://localhost:4502/editor.html${config.aemContentPath}/${config.component.name}.html`
          );
        } else {
          log(`Stories creation failed. :(`);
        }
      } else {
        log(`Creating JCR:Content for '${config.component.name}' component failed.`);
      }
    } else {
      log(`Component page WAS NOT created for '${config.component.name}' component.`);
    }
  } else {
